...
async def addadmin(client,message):
    ...
    await admins_db.insert_one({"user_id":uid})
...
async def removeadmin(client,message):
    if message.from_user.id!=ADMIN_ID:
        return
    uid=int(message.command[1])
    await admins_db.delete_one({"user_id":uid})
...
async def detect_storage(client,message):
    ...
    if cid not in course_buffers:
        course_buffers[cid]=[]

    course_buffers[cid].append((course,message))

async def upload_worker():

    while True:

        for cid in list(course_buffers.keys()):

            buffer=course_buffers[cid]
