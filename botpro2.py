...
async def addprotect(c, m):
    ...
    if await channels.find_one({"storage": storage}):
        await m.reply("Storage already registered")
        return
...
async def protectcleandb(c, m):
    ...
    await m.reply("Database cleaned")
...
async def reconnect(c, m):
    ...
    async for x in channels.find():
        ...
        try:
            await bot.get_chat(x["storage"])
            await bot.get_chat(x["public"])
            text += f"✅ {x['name']}\n"
        except:
            text += f"❌ {x['name']}\n"
...
async def detect(c, m):
    ...
    await queue.put((course["public"], m.id, btn))
