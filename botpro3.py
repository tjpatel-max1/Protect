...
async def check_spam(user_id, name):

    now = time.time()

    if user_id in user_timeout and now < user_timeout[user_id]:
        return int(user_timeout[user_id] - now)
        ...
        if level == 0:
            ...
            try:
                await bot.send_message(
                    ...
                    f"User: {name}\n"
                    f"ID: {user_id}\n"
                    f"Triggered repeated spam limit."
                )
            except:
                pass

        user_timeout[user_id] = now + timeout
        return timeout
...
async def start(client,message):
    ...
    # ADMIN BYPASS FOR ANTISPAM
    if not await is_admin(user_id):

        wait = await check_spam(user_id, message.from_user.first_name)
